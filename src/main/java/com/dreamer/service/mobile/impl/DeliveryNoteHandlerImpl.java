...
public class DeliveryNoteHandlerImpl extends BaseHandlerImpl<DeliveryNote> implements DeliveryNoteHandler {
    ...
    private Double getLogisticsFee(DeliveryNote note) {
        if(note.getAddress()!=null&&!note.getAddress().getCountry().equals("中国"))return 0.0;//不是中国不要物流费
...
//        noticeHandler.noticeTransfer(transfer);
    ...
    public void deliveryGoods(AddressMy address, Integer fromUid, Integer toUid, Integer[] goodsIds, Integer[] amounts, String remark) {
        ...
        if(toAgent.getAgentStatus().getDesc().equals("整顿")){
        	throw new ApplicationException("接收人已处于整顿状态,请联系客服!");
        }
        //提交申请
        applyDeliveryNote(deliveryNote);
        //减少库存
        ...
        //待发货通知
        noticeHandler.noticeDeliveryNote(deliveryNote);
    }
    ...
    public void delivery(Integer noteId, String company, String code, Double actual_logisticsFee) {
        ...
        if (!note.getStatus().equals(DeliveryStatus.DELIVERY)) {
            ...
            if (actual_logisticsFee > note.getLogisticsFee()) {//实际物流费要多一些
                ...
                accountsRecords.add(accountsHandler.deductAccountAndRecord(AccountsType.PURCHASE, note.getApplyAgent(), note.getToAgent(), fee, more));
            }
            if (actual_logisticsFee < note.getLogisticsFee()) {//实际物流费要多一些
                ...
                accountsRecords.add(accountsHandler.increaseAccountAndRecord(AccountsType.PURCHASE, note.getApplyAgent(), note.getToAgent(), fee, more));
            }
