...
public class OrderHandlerImpl extends BaseHandlerImpl<Order> implements OrderHandler {
    ...
    public Order commitOrder(Agent agent, AddressClone addressClone, ShopCart shopCart, String remark) {
        ...
        for(OrderItem orderItem : items){
        	if(orderItem.getPmallGoods()!=null){
        		PmallGoods pmallGoods = orderItem.getPmallGoods();
            	if(3021==pmallGoods.getId()||!pmallGoods.getNeedFee()){
            		flag=false;
            	}else {
            	    flag =true;
                }
    ...
    private void payOrder(Integer id, PaymentWay paymentWay, Integer cash) {
        ...
        if (myA < tem) {//券不够用
            ...
            if (tem > 0) {//需要扣券
                ...
                payOrder(order, paymentWay, 0.0);//成功支付
            }
...
//            addRestraction(user,"agentCode",p.getEntity().getUser().getAgentCode());
    ...
    private void validateActitivty(PmallGoods pmallGoods, Agent agent, Integer quantity) {
        ...
        if (!agent.getLevelName().equals("联合股东")){
        	throw new ApplicationException("提交失败," + pmallGoods.getName() + "只允许联合股东级别购买");
        }
...
//        }
        ...
        order.getItems().forEach(item -> {
            CommonUtil.putAll(maps, accountsHandler.rewardPmallVoucher(agent, item.getPmallGoods(), item.getQuantity()),true);
        });
...
//            System.out.println(agent+"==="+map.get(agent));
            accountsRecords.add(accountsHandler.increaseAccountAndRecord(AccountsType.PURCHASE, agent, order.getUser(), map.get(agent), sb.toString()));
        }
...
//        return 0.0;
	...
	public void updateDeliverInfo(Integer id, String logisticsCode,
			String logistics) {
		orderItemDao.updateDeliverInfo(id,logisticsCode,logistics);
	}
}
