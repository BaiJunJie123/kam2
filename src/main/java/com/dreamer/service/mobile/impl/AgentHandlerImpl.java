...
public class AgentHandlerImpl extends BaseHandlerImpl<Agent> implements AgentHandler {
    ...
    public Agent selfRegister(Agent agent, String refCode) {
        ...
        if (tem != null && tem.getAgentCode() == null) {//判断当前用户是否存在，存在的话是扫二维码过来的,完信息
...
//        visitor.setAgentCode(createAgentCode());
    ...
    public String createAgentCode() {
        if(codeList==null){//找出所有的编码 每次找一次
            codeList = agentDao.findAllCode();
        }
        String code = randomCode();
        //随机直到不产生相同的为止
        while (codeList.contains(code)||copyOnWriteArrayList.contains(code)){
           code = randomCode();
        }
        copyOnWriteArrayList.add(code);//加入产生的
        return code;
//        String baseCode = "kam";
//        Integer code = 800000;
//        String startCode = baseCode + code;
//        List<String> agentCodes = (List<String>) agentDao.findAgentCodesAfter(startCode);
//        for (int i = 0; i < 99999; i++) {
//            Integer nowCode = code + i;
//            String str = baseCode+String.valueOf(nowCode);
//            //超出上限都不存在
//            if(i>=agentCodes.size()){
//                return str;
//            }
//            if (!agentCodes.get(i).equals(str)) {
//                return str;
//            }
//
//        }
...
//        System.out.println(str.substring(str.length()-s,str.length()-e));
    ...
    private Agent findByAgentCodeOrMobile(String name) {
        ...
        //验证登陆
        if (list == null || list.isEmpty()) {
            throw new ApplicationException("不存在此用户" + name);
        } else if (list.size() > 1) {
...
//        }
    ...
    @Override
    public Boolean isBoss(Integer aid, Integer pid) {
        Agent boss = findBoss(aid);
        if (boss != null && boss.getId().equals(pid)) {
            return true;
        }
        return false;
    }
    ...
    String[] vips = new String[]{"kam368173", "kam122855", "kam915011", "kam615652", "kam750519", "kam777777", "kam308358", "kam253729", "kam399555", "kam579516", "kam953828", "kam089162", "kam998087"};

    /**
     * 是否能转券  vip能给自己下面的所有人转券&&vip只能由公司转给他们
     * 其股东只能抓给自己下面的人  团队下面的人可以转给他    isteam就可以了
     * 其他人可以转给自己股东  可以转给自己下面的人  退货   isteam是个团队就可以了
     *
     * @param aid
     * @param uid
     * @return
     */
    @Override
    public Boolean canTransferAccounts(Integer aid, Integer uid) {
        if (aid.equals(uid)) return true;
        Agent agent = get(aid);
        Agent user = get(uid);
        //接收人是否是vip
        Boolean isAVip = Arrays.asList(vips).contains(agent.getAgentCode());
        //转出人是否vip
        Boolean isUVip = Arrays.asList(vips).contains(user.getAgentCode());
        //如果接收人是vip
        if (isAVip) {
            //如果是公司的话 可以转
            if (user.isMutedUser()) {
    ...
    @Override
    public Boolean canTransferGoods(Integer aid, Integer uid) {
        if (aid.equals(uid)) return true;
        Agent agent = get(aid);
        Agent user = get(uid);
        //接收人是否是vip
    ...
    @Override
    public Boolean isVipBoss(Integer aid) {
        Agent agent = get(aid);
        Boolean isAVip = Arrays.asList(vips).contains(agent.getAgentCode());
        return isAVip;
    }
    ...
    public Object findNamePidByAgentCode(String agentCode) {
        return agentDao.findNamePidByAgentCode(agentCode);
    }
