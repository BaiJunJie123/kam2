...
public class OrderPayController {
	...
	//修改--“商城订单-发货管理”物流
		@RequestMapping(value = "/editOrderDeliverInfo.json", method = RequestMethod.POST)
		public Message delivery( DeliveryNote parameter,HttpServletRequest request) {
			try {
				User user=(User)WebUtil.getCurrentUser(request);
				if(!user.isAdmin()){
						throw new ApplicationException("非管理员无修改权限");
				}
				orderHandler.updateDeliverInfo(parameter.getId(),parameter.getLogisticsCode(),parameter.getLogistics());
