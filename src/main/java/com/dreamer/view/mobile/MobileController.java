...
public class MobileController {
    ...
    public String toRegister(@RequestParam(required = false) String s_unionid, @RequestParam(required = false) String s_openid, @RequestParam(required = false) String refCode, Model model,HttpServletRequest request) {
//        Object ob = WebUtil.getCurrentUser(request);
        //TODO 这里有问题
        if (s_openid != null) {
...
//        String refCode = (String) request.getSession().getAttribute("refCode");
        if (refCode == null || refCode == "") return Message.createFailedMessage("没有推荐人，请联系管理员");
...
//    }
    ...
    public String profile(Integer uid, Model model, HttpServletRequest request) {
        ...
        Boolean canTA = agentHandler.canTransferAccounts(uid,user.getId());
        Boolean canTG = agentHandler.canTransferGoods(uid,user.getId());
        model.addAttribute("canTA",canTA);
        model.addAttribute("canTG",canTG);
        model.addAttribute("vipBoss",agentHandler.isVipBoss(uid));
        return "mobile/profile";
    }
    ...
    public String to_recharge(Model model, HttpServletRequest request) {
        ...
        Agent agent = agentHandler.get(user.getId());
        model.addAttribute("toAgent", agent);
        model.addAttribute("balance", agent.getAccounts().getAccount(AccountsType.PURCHASE));
...
//        String orderNo = CommonUtil.createNoByTime()+"_"+agent.getAgentCode();
        ...
        try {
            ...
            AccountsTransfer accountsTransfer = new AccountsTransfer(agent, fromAgent, "物流费充值", amount, AccountsType.PURCHASE, new Date());
...
//        return Message.createSuccessMessage();
    ...
    @Autowired
    private com.dreamer.service.user.UserHandler userHandler;
