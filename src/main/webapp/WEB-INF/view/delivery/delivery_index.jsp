							...
							<tbody id="dataList">
								<c:forEach items="${dls}" var="g">
									<tr data-row="${g.id}">
										...
										<td>${g.status.desc}</td>
										<td>
											<a class="btn btn-success default" href="<c:url value='/securityCode/scan_num.html?owner=${g.toAgent.realName}&&agentCode=${g.toAgent.agentCode}' />"><span class="fa fa-wrench fa-fw" aria-hidden="true"></span>录防伪码</a>
										<c:choose>
										<c:when test="${user.agent}">
										...
										<c:when test="${user.admin&&user.loginName ne 'zmz365zmz'}">
											...
											<c:if test="${g.status=='CONFIRM' or (g.status=='NEW' and g.applyOrigin=='SYSTEM') or g.status == 'DELIVERING' }">
											<a class="btn btn-success default editBtn" 
