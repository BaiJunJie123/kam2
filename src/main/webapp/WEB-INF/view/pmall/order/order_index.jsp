                    ...
                    <form class="navbar-form navbar-left" id="searchForm"
                        ...
                        <div class="form-group">
                            <label class="">订单号</label> <input type="text"
                                                               value="${parameter.entity.orderNo}" name="entity.orderNo"
                                                               id="orderNo"
                                                               autofocus class="typeahead form-control"
                                                               placeholder="订单号查询">
                            <label class="">收货人</label> <input type="text"
                                                               value="${parameter.entity.addressClone.consignee}"
                                                               name="entity.addressClone.consignee" id="consignee"
                                                               autofocus class="typeahead form-control"
                                                               placeholder="收货人姓名">

                            <label class="">下单人</label> <input type="text"
                                                               value="${parameter.entity.user.realName}"
                                                               name="entity.user.realName" id="realName"
                                                               autofocus class="typeahead form-control"
                                                               placeholder="下单人姓名">

                            <label class="">联系电话</label> <input type="text"
                                                               value="${parameter.entity.addressClone.mobile}" name="entity.addressClone.mobile" id="mobile"
                            ...
                            <select name="entity.paymentStatus" id="paymentStatus" class="form-control">
                                ...
                                <c:forEach items="${paymentStatus}" var="s">
                                    <option value="${s}"
                                            <c:if test="${parameter.entity.paymentStatus==s}">selected</c:if>>
                                            ${s.name}</option>
                                </c:forEach>
                            </select>
                    ...
                    <table
                        ...
                        <c:forEach items="${orders}" var="g">
                            ...
                            >
                                ...
                                <td>${g.orderNo}</td>
                                <td>${g.quantity}</td>
                                <td>${g.amount}+${g.ticket}</td>
                                <td>${g.user.realName}</td>
                                <td>${g.user.agentCode}</td>
                                <%-- <td>${g.addressClone.consignee}</td>
                                <td>${g.addressClone.mobile}</td> --%>
                                <td>${g.orderTime}</td>
                                ...
                                <td>
                                    <c:if test="${user.admin&&user.loginName ne 'zmz365zmz'}">
                                        <c:if test="${g.paymentStatus=='UNPAID' }">
                                            <a class="btn btn-success default  ajaxLink" href="<c:url value='/pmall/shopcart/adminPay.json?id=${g.id}' /> "><span
                                                    class="glyphicon glyphicon-ok fa-fw"
                                                    aria-hidden="true"></span>付款</a>
                                            <a class="btn btn-danger default editBtn"
                                               href="<c:url value='/pm/order/revoke.html?id=${g.id}' /> "><span
                                                    class="fa fa-trash fa-fw" aria-hidden="true"></span>撤销</a>
                                        </c:if>
                                        <%--<c:if test="${g.status=='SHIPPED' }">--%>
                                            <%--<a class="btn btn-info default ajaxLink"--%>
                                               <%--href="<c:url value='/pm/order/receive.json?id=${g.id}' /> "><span--%>
                                                    <%--class="glyphicon glyphicon-ok fa-fw"--%>
                                                    <%--aria-hidden="true"></span>收货</a>--%>
                                        <%--</c:if>--%>
                                        <%--<c:if test="${g.paymentStatus=='PAID' and (g.status=='RETURNED' or g.status=='REVOKED') }">--%>
                                            <%--<a class="btn btn-danger default editBtn"--%>
                                               <%--href="<c:url value='/pm/order/refund.html?id=${g.id}' /> "><span--%>
                                                    <%--class="fa fa-reply-all fa-fw" aria-hidden="true"></span>退款</a>--%>
                                        <%--</c:if>--%>
                                        <%--<c:if test="${g.status=='RECEIVED' }">--%>
                                            <%--<a class="btn btn-warning default editBtn"--%>
                                               <%--href="<c:url value='/pm/order/return.html?id=${g.id}' /> "><span--%>
                                                    <%--class="fa fa-reply fa-fw" aria-hidden="true"></span>退货</a>--%>
                                        <%--</c:if>--%>
                                        <a class="btn btn-success default editBtn"
                                           href="<c:url value='/pm/order/detail.html?id=${g.id}' /> "><span
                                                class="glyphicon glyphicon-list fa-fw" aria-hidden="true"></span>详情</a>
                                    </c:if>
